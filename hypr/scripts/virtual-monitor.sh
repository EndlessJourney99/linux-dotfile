...
# Now switch config of hyprland monitor to use ipad VIRTUAL_MONITOR
#
sed -i 's/2-monitors/ipad-virtual-screen/g' ~/.config/hypr/conf/monitor.conf
...
cleanup() {
  ...
  pkill wayvnc
  echo "[wayvnc] Done."
  sed -i 's/ipad-virtual-screen/2-monitors/g' ~/.config/hypr/conf/monitor.conf
  echo "switch monitors config to normal"
  exit 0
}
...
wayvnc --gpu -R 0.0.0.0 5900 "$VIRTUAL_MONITOR"
